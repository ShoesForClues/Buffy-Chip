...
function API:merge_tables_deep(...)
	local tables={...}
	local new_table=tables[1] or {}
	
	for _,t in pairs(tables) do
		for i,v in pairs(t) do
			if new_table[i]==nil then
				new_table[i]=v
			elseif type(new_table[i])=="table" and type(v)=="table" then
				new_table[i]=API:merge_tables_deep(new_table[i],v)
			end
		end
	end
