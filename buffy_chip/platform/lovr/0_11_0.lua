...
function platform:get_running_platform()                                                           --Get the platform specs
	return {operating_system=lovr.getOS();bits=32;}
end
...
function platform:exit() lovr.event.quit() end                                                     --End the program
	...
	for a=1,path_length do
		...
		if char~="/" and char~="<" and a<path_length then
			...
			if step<=0 and file_name=="root" then
				if stdplib.root_functions.string.find(stdplib.root_functions.string.lower(lovr.system.getOS()),"windows") or stdplib.root_functions.string.find(stdplib.root_functions.string.lower(lovr.system.getOS()),"dos") then
					current_file="C:"
				else
					current_file="/"
				end
...
function platform:file_exists(file)                                                                --Check if file exists
	return lovr.filesystem.isFile(platform:get_file(file))
end
function platform:get_sub_files(file)                                                              --Get a list of sub files
	return lovr.filesystem.getDirectoryItems(platform:get_file(file))
end
function platform:get_full_path(file)                                                              --Get full path of file
	return lovr.filesystem.getRealDirectory(platform:get_file(file))
end
function platform:read_lines(file)
	local lines={}
	for line in lovr.filesystem.read(platform:get_file(file)) do
		lines[#lines+1]=line
	end
	return lines
end
function platform:create_file(file_name)
	...
	local source,error=lovr.filesystem.newFile(platform:get_file(file_name))
...
function platform:yield(duration) lovr.timer.sleep(duration) end                                   --Yield the entire thread
function platform:get_tick() return os.clock()-platform.start_tick end                             --Get the current clock tick
function platform:get_frame_rate() return lovr.timer.getFPS() end                                  --Get the current FPS

function platform:get_joystick_key_press(joystick_id,...)                                          --Check if joystick button is pressed
	local state=false
	for id,joystick in pairs(lovr.joystick.getJoysticks()) do
...
function platform:get_key_press(...) return lovr.keyboard.isDown(...) end                          --Check if keyboard button is pressed
function platform:get_mouse_key_press(...) return lovr.mouse.isDown(...) end                       --Check if mouse button is pressed
function platform:update_pointers()                                                                --Update pointers
	local pointers={}
	local mouse_x,mouse_y=lovr.mouse.getPosition()
	--[[
	if lovr.mouse.isDown(1)==true and lovr.mouse.isDown(2)==true then
		table.insert(pointers,#pointers+1,{id=1,position={x=mouse_x,y=mouse_y}})
	end
	--]]
	for _,id in pairs(lovr.touch.getTouches()) do
		local touch_x,touch_y=lovr.touch.getPosition(id)
...
function platform:get_max_resolution()                                                             --Get max supported screen resolution
	local modes=lovr.window.getFullscreenModes()
...
function platform:load_source(file,file_type,properties)                                           --Load asset in memory for future reference
	...
	if file_type==platform.enum.file_type.image then
		if platform.assets[file]==nil then
			success,platform.assets[file]=pcall(lovr.graphics.newImage,platform:get_file(file))
			if success==false then
	...
	elseif file_type==platform.enum.file_type.font then
		if platform.assets[file]==nil then
			local font={}
			for i=6,108 do
				success,font[i]=pcall(lovr.graphics.newFont,platform:get_file(file),i)
				if success==false then
	...
	elseif file_type==platform.enum.file_type.audio then
		if platform.assets[file]==nil then
			success,platform.assets[file]=pcall(lovr.sound.newSoundData,platform:get_file(file))
			if success==false then
...
function platform:create_buffer(...)                                                               --Create an external buffer
	local args={...}
	local buffer=lovr.graphics.newCanvas(...)
	return buffer
end
...
function platform:clear_buffer(color,opacity,active_buffers)                                       --Clears buffer
	lovr.graphics.clear({color.r,color.g,color.b,(1-opacity)},unpack(active_buffers))
end
...
function platform:render_image(source,position,size,rotation,wrap,background_color,source_color,filter_mode,anistropy,buffer) --Render image to the buffer
	if position==nil or size==nil or source==lovr.graphics.getCanvas() then return end
	if wrap~=nil then
		if position.x+size.x<wrap.x1 or position.x>wrap.x2 or position.y+size.y<wrap.y1 or position.y>wrap.y2 then return end
		lovr.graphics.setScissor(wrap.x1,wrap.y1,wrap.x2-wrap.x1,wrap.y2-wrap.y1)
	end
	lovr.graphics.setColor(background_color.r,background_color.g,background_color.b,(1-background_color.a))
	lovr.graphics.rectangle("fill",position.x,position.y,size.x,size.y)
	if source~=nil then
		...
		lovr.graphics.setColor(source_color.r,source_color.g,source_color.b,(1-source_color.a))
		lovr.graphics.draw(
			source,
...
function platform:render_text(text,position,wrap,wrapped,color,alignment,font,font_size,buffer)    --Render text to the screen
	...
	if color~=nil then
		lovr.graphics.setColor(color.r,color.g,color.b,(1-color.a))
	end
	if font[font_size]~=nil then
		font[font_size]:setFilter(platform.enum.filter_mode.nearest,platform.enum.filter_mode.nearest,0)
		lovr.graphics.setFont(font[font_size])
		lovr.graphics.printf(text,position.x,position.y,wrap.x2-wrap.x1,"left")
	end
	lovr.graphics.setScissor()
	lovr.graphics.setColor(1,1,1,1)
end
...
function platform:create_audio(properties)                                                         --Create an audio source
	local audio_object={
		source=lovr.audio.newSource(properties.source);
	}
	function audio_object:set_source(source)
		audio_object.source=lovr.audio.newSource(source)
	end
...
function platform:set_error_handler(handler)
	function lovr.errorhandler(error_message)
...
function platform:set_window_position(position)
	if position~=nil then
		lovr.window.setPosition(position.x,position.y,1)
	end
end
...
function platform:initialize(properties)
	...
	--::::::::::::::::::::[Callbacks]::::::::::::::::::::
	--[[
	function lovr.keypressed(key) platform.key_state:invoke({key=key,state=true}) end
	function lovr.keyreleased(key) platform.key_state:invoke({key=key,state=false}) end
	function lovr.joystickpressed(joystick,key)
		local id,_=joystick:getID()
		platform.joystick_key_state:invoke({id=id or 1,key=key,state=true})
	end
	function lovr.joystickreleased(joystick,key)
		local id,_=joystick:getID()
		platform.joystick_key_state:invoke({id=id or 1,key=key,state=false})
	end
	function lovr.textinput(text) platform.text_input:invoke(text) end
	function lovr.touchpressed(id,x,y) platform:update_pointers() end
	function lovr.touchreleased(id,x,y) platform:update_pointers() end
	function lovr.mousepressed(x,y,id)
		platform.mouse_key_state:invoke({key=id,state=true})
	end
	function lovr.mousereleased(x,y,id)
	...
	function lovr.touchmoved() platform:update_pointers() end
	--]]
	function lovr.update(...) platform.update_stepped:invoke(...) end
	--[[
	function lovr.resize(x,y) platform.screen_resolution:set_value({x=x,y=y}) end
	function lovr.wheelmoved(x,y) platform.wheel_scrolled:invoke({x=x,y=y}) end
	--]]
	platform:get_buffer_resolution()

	for i=6,72 do
		_,platform.default.font[i]=pcall(lovr.graphics.newFont,i)
	end
	...
	function lovr.draw(...)
		...
		--lovr.graphics.print("FPS: "..tostring(platform:get_frame_rate()))
	end
end
--_____________________________________________________________________________
