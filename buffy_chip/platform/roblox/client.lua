...
function platform:info(message) if message==nil then return end                                    --Output info to console
	platform:print("[Client]["..stdplib:get_time_stamp(platform:get_tick()).."]: "..tostring(message))
end
...
function platform:get_key_press(...)	                                                            --Check if keyboard button is pressed
	local keys={...}	
	local state=false
	for _,key in pairs(keys) do
		if user_input_service:IsKeyDown(key) then
			state=true
			break
		end
	end
...
function platform:load_source(file,file_type,properties)                                      --Load asset in memory for future reference
	...
	if success==false or source==nil then
		platform:info("Failed to load: "..tostring(file))
	end
	return source
end
...
function platform:initialize(properties)
	...
	user_input_service.InputBegan:connect(function(input,game_processed_event)
		if game_processed_event==true then return end
		if input.UserInputType==Enum.UserInputType.Keyboard then
			 platform.key_state:invoke({key=input.KeyCode,state=true,current_text_box=user_input_service:GetFocusedTextBox()})
		elseif input.UserInputType==Enum.UserInputType.MouseButton1 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton1,state=true})
		elseif input.UserInputType==Enum.UserInputType.MouseButton2 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton2,state=true})
		elseif input.UserInputType==Enum.UserInputType.MouseButton3 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton3,state=true})
		end
	end)
	user_input_service.InputEnded:connect(function(input,game_processed_event)
		if game_processed_event==true then return end
		if input.UserInputType==Enum.UserInputType.Keyboard then
			 platform.key_state:invoke({key=input.KeyCode,state=false,current_text_box=user_input_service:GetFocusedTextBox()})
		elseif input.UserInputType==Enum.UserInputType.MouseButton1 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton1,state=false})
		elseif input.UserInputType==Enum.UserInputType.MouseButton2 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton2,state=false})
		elseif input.UserInputType==Enum.UserInputType.MouseButton3 then
			platform.mouse_key_state:invoke({key=Enum.UserInputType.MouseButton3,state=false})
		end
	end)
	user_input_service.InputChanged:connect(function(input,game_processed_event)
		if game_processed_event==true then return end
		...
		elseif input.UserInputType==Enum.UserInputType.MouseWheel then
			platform.mouse_scrolled:invoke(input.Position.Z)
		end
	end)
	run_service.Heartbeat:Connect(function(...) platform.update_stepped:invoke(...) end)
	
	platform.render_stepped:attach_bind(function()
		if platform.screen_resolution.value~=camera.ViewportSize then
			platform.screen_resolution:set_value(camera.ViewportSize)
		end
	end)
	...
	if run_service:IsClient() then
		...
		run_service:BindToRenderStep("main_bind",Enum.RenderPriority.First.Value,platform.render_stepped.invoke)
