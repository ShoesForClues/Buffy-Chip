...
return function(thread)
	...
	function API:get_lines(text)
		...
		while x<thread.libraries["stdlib"].root_functions.string.len(text) do
			...
			if not a then
				...
				lines[#lines+1]=thread.libraries["stdlib"].root_functions.string.sub(text,a,b-1)
			end
			x=b+1
		end;
		return lines
	...
	function API:get_name(path)
		...
		return thread.libraries["stdlib"].root_functions.string.sub(path,(path:match("^.*()/") or 0)+1,thread.libraries["stdlib"].root_functions.string.len(path))
	end
